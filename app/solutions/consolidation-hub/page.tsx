...
const capabilities = [
  {
    ...
    desc: "Assign individual packages to pallets with full weight and dimension tracking. Each pallet has its own record showing current load, assigned container, and departure status.",
  },
  {
    icon: Cube,
    title: "Container Fill Optimization",
    desc: "Visualize available container capacity before finalizing a load. The system shows weight and volume utilization and flags containers that are over or under capacity.",
  },
  {
    icon: ArrowsLeftRight,
    title: "Load Planning",
    desc: "Plan how packages are arranged across pallets and containers. Organize by destination, route, or shipment type to minimize handling time at discharge ports.",
  },
  {
    icon: Barcode,
    title: "Barcode Scan to Consolidate",
    desc: "Warehouse staff scan barcodes at the consolidation station to assign packages to consolidation batches. Each scan is logged with timestamp and operator identity.",
  },
  {
    icon: ClipboardText,
    title: "Manifest Workflows",
    desc: "Generate voyage manifests automatically from consolidation batch data. Manifests include all package details, weights, dimensions, and customs information for carriers.",
  },
  {
    icon: Boat,
    title: "Voyage Assignment",
    desc: "Assign consolidation batches to voyages in one step. Track the voyage from manifest creation through departure confirmation and arrival at destination port.",
  },
  {
    icon: Warehouse,
    title: "Warehouse Location Integration",
    desc: "Consolidated cargo stays linked to its warehouse shelf and bin location until dispatch. Staff always know where any item is in the warehouse during the consolidation process.",
  },
  {
    icon: ChartLine,
    title: "Hub Performance Reports",
    desc: "Monitor consolidation volumes, container fill rates, average dwell time per shipment, and departure schedules. Export reports for operations and management teams.",
  },
];
...
const workflow = [
  {
    ...
    desc: "Incoming shipments arrive at the consolidation hub and are scanned into the system. Each package is assigned a bin location and flagged for its destination consolidation batch.",
  },
  {
    step: "02",
    title: "Sort and Group",
    desc: "Packages are sorted by destination route and grouped into consolidation batches. The system shows how many packages are ready for each batch and their combined weight.",
  },
  {
    step: "03",
    title: "Assign to Pallets",
    desc: "Staff scan packages to assign them to pallets. Load optimization tracks remaining capacity and alerts when a pallet reaches its weight or volume limit.",
  },
  {
    step: "04",
    title: "Load into Containers",
    desc: "Pallets are assigned to containers. The system shows container fill levels and confirms the load is within capacity before the manifest is generated.",
  },
  {
    step: "05",
    title: "Generate and Dispatch",
    desc: "Manifest is produced automatically. Staff confirm loading is complete and close the manifest. The voyage status updates and the consolidation batch is dispatched.",
  },
];
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background text-foreground transition-colors duration-300">
      ...
      <main className="flex-grow pt-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 border-l border-border-custom">
              {stats.map((stat, i) => (
                <AnimatedSection
                  key={i}
                  delay={i * 0.1}
                  className="p-6 border-r border-b md:border-b-0 border-border-custom text-center"
                >
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-8">
            <AnimatedSection className="text-left mb-12 max-w-2xl">
              ...
              <h2 className="text-2xl font-bold font-sans uppercase tracking-tight">
                Full Consolidation Hub Control
              </h2>
              <p className="text-sm text-foreground/70 font-sans leading-relaxed font-medium mt-3">
                Every step from receiving shipments at the hub to loading the final container and generating the voyage manifest, tracked and automated.
              </p>
            </AnimatedSection>
        ...
        <section className="py-16 border-b border-border-custom bg-primary/[0.01]">
          <div className="mx-auto max-w-7xl px-6 md:px-8">
            <AnimatedSection className="text-left mb-12 max-w-2xl">
              ...
              <h2 className="text-2xl font-bold font-sans uppercase tracking-tight">
                From Hub Receiving to Voyage Dispatch
              </h2>
              <p className="text-sm text-foreground/70 font-sans leading-relaxed font-medium mt-3">
                A clear five-step hub consolidation cycle from the moment freight arrives to when the container departs.
              </p>
            </AnimatedSection>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 border-t border-l border-border-custom relative">
              ...
              {workflow.map((step, index) => (
                <AnimatedSection
                  ...
                  delay={index * 0.12}
                  direction="up"
                  className="p-6 border-r border-b border-border-custom hover:bg-primary/[0.02] transition-all relative"
                >
                  <motion.div
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    ...
                    className="w-8 h-8 rounded-full bg-[var(--console-bg)] border border-blue-500/30 flex items-center justify-center mb-3"
                  >
        ...
        {/* Manifest Features + Pallet Record */}
        <section className="py-16 border-b border-border-custom">
          <div className="mx-auto max-w-7xl px-6 md:px-8 grid md:grid-cols-2 gap-8">
            ...
            <AnimatedSection direction="left" className="bg-[var(--console-bg)] border border-border-custom rounded-none p-6 space-y-4">
              ...
              <p className="text-sm text-foreground/70 font-sans leading-relaxed font-medium">
                Voyage manifests are produced automatically from consolidation batch data. No re-entering information. The manifest pulls directly from the shipment and pallet records.
              </p>
            ...
            <AnimatedSection direction="right" className="bg-[var(--console-bg)] border border-border-custom rounded-none p-6 space-y-4">
              ...
              <p className="text-sm text-foreground/70 font-sans leading-relaxed font-medium">
                Each pallet in the hub has a complete operational record. Staff can locate, weigh, and dispatch any pallet without guesswork.
              </p>
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 border-t border-l border-border-custom">
              {[
                {
                  icon: Boat,
                  title: "Ocean Freight Consolidators",
                  desc: "Receive LCL cargo from multiple shippers, sort by port of destination, load into FCL containers, and generate Bills of Lading and port manifests in one workflow.",
                },
                {
                  icon: Buildings,
                  title: "Transit Warehouse Hubs",
                  desc: "Manage high-volume receiving from multiple origins. Sort, palletize, and load outbound containers while tracking every package through barcode scanning at each stage.",
                },
                {
                  icon: Truck,
                  title: "Regional Distribution Centers",
                  desc: "Consolidate last-mile deliveries from a central hub into route-based loads. Maximize vehicle capacity and reduce the number of runs with load optimization tools.",
                },
              ].map((item, i) => (
              ...
              {[
                {
                  ...
                  desc: "Configure warehouse shelf and bin locations, manage capacity, and run full receiving and dispatch operations that feed into the consolidation hub.",
                  href: "/solutions/warehouse-management",
                },
                {
                  title: "Inventory and Stock Control",
                  desc: "Track every package at item level with barcode scanning and real-time stock updates that integrate directly with consolidation batch assignments.",
                  href: "/solutions/inventory-control",
                },
                {
                  title: "Customs and Compliance",
                  desc: "Generate customs documentation for consolidated shipments and run compliance screening before the manifest is submitted to port authorities.",
                ...
                <AnimatedSection key={i} delay={i * 0.1}>
                  <Link
                    href={sol.href}
                    className="block p-6 border-r border-b border-border-custom hover:bg-primary/[0.04] hover:border-blue-500/30 transition-all group h-full"
                  >
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-8 relative z-10">
            <AnimatedSection className="max-w-2xl space-y-4">
              <h2 className="text-2xl md:text-3xl font-extrabold uppercase font-sans tracking-tight leading-none">
                ...
                <br />
                <span className="text-blue-500">Consolidation Hub?</span>
              </h2>
              <p className="text-sm text-foreground/70 font-sans leading-relaxed font-medium">
                See how NTIGI handles pallet tracking, load optimization, and manifest generation for your consolidation hub in a live walkthrough with our team.
              </p>
              <div className="flex flex-wrap gap-3 pt-2">
                <Button variant="primary" href="/demo" size="lg">Request a Demo</Button>
